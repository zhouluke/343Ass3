...
<skills>
{
	let $allReqSkills:= doc("posting.xml")//reqSkill/@what
	for $reqSkill in fn:distinct-values($allReqSkills)
	return <skill>
		...
		{
			let $s := doc("resume.xml")//resume/skills//*[@what=$reqSkill]
			let $four := fn:count($s/.[@level=4]/@level)
			return <count>
				{attribute level {4}}
				{attribute n {$four}}
			</count>
		...
		{
			let $s := doc("resume.xml")//resume/skills//*[@what=$reqSkill]
			let $five := fn:count($s/.[@level=5]/@level)
			return <count>
				{attribute level {5}}
				{attribute n {$five}}
			</count>
